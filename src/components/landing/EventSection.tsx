            ...
            <div className="container mx-auto px-4">
                ...
                <div className="max-w-4xl mx-auto mb-16">
                    ...
                    <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                        <div className="bg-gradient-to-r from-pink-500 to-pink-600 text-white p-8 text-center">
                            <div className="text-lg opacity-90 mb-2">{settings.eventTimeLabel || 'Thời gian'}</div>
                        ...
                        <div className="p-8">
                            <div className="grid md:grid-cols-2 gap-8">
                                <div>
                                    <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                        <span className="text-2xl">📍</span> {settings.eventLocationLabel || 'Địa điểm'}
                                    </h4>
                                ...
                                <div>
                                    <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                        <span className="text-2xl">👥</span> {settings.eventParticipantsLabel || 'Thành phần'}
                                    </h4>
                                    <p className="text-gray-600">
                            ...
                            <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                                <span className="text-2xl">📋</span> {settings.eventProgramLabel || 'Nội dung chương trình'}
                            </h4>
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="text-center mb-8">
                        <span className="text-5xl mb-4 block">📊</span>
                        <h2 className="font-heading text-3xl md:text-4xl font-bold text-gray-800 mb-4">
                            {settings.financeTitle || 'Cơ Cấu Tài Chính'}
                        </h2>
                        <p className="text-lg text-gray-600">{settings.financeSubtitle || 'MINH BẠCH TUYỆT ĐỐI'}</p>
                    </div>

                    <div className="bg-white rounded-2xl shadow-xl p-8">
                        <div className="text-center mb-8">
                            <div className="text-sm text-gray-500 mb-2">{settings.financeTotalLabel || 'Tổng thu dự kiến'}</div>
                            <div className="text-4xl font-bold text-pink-600">{settings.financeTotalAmount || '500.000.000 VNĐ'}</div>
                        </div>
                        ...
                        <div className="grid md:grid-cols-3 gap-6">
                            ...
                            <div className="relative">
                                <div className="absolute -top-3 left-4 bg-green-500 text-white text-sm font-bold px-3 py-1 rounded-full">
                                    {settings.financeTreePercent || '60%'}
                                </div>
                                <div className="bg-green-50 rounded-xl p-6 border-2 border-green-200 pt-8">
                                    <div className="text-3xl mb-3">🌱</div>
                                    <h4 className="font-bold text-gray-800 mb-2">{settings.financeTreeTitle || 'Chi Phí Cây Giống'}</h4>
                                    <div className="text-2xl font-bold text-green-600 mb-2">{settings.financeTreeAmount || '300 Triệu'}</div>
                                    <p className="text-sm text-gray-600">
                                        {settings.financeTreeDesc || 'Mua 200 cây Mai Anh Đào trưởng thành đạt tiêu chuẩn "Cây Di sản"'}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="relative">
                                <div className="absolute -top-3 left-4 bg-blue-500 text-white text-sm font-bold px-3 py-1 rounded-full">
                                    {settings.financeCarePercent || '30%'}
                                </div>
                                <div className="bg-blue-50 rounded-xl p-6 border-2 border-blue-200 pt-8">
                                    <div className="text-3xl mb-3">🛡️</div>
                                    <h4 className="font-bold text-gray-800 mb-2">{settings.financeCareTitle || 'Quỹ Chăm Sóc Cây'}</h4>
                                    <div className="text-2xl font-bold text-blue-600 mb-2">{settings.financeCareAmount || '150 Triệu'}</div>
                                    <p className="text-sm text-gray-600">
                                        {settings.financeCareDesc || 'Thuê Công ty Công trình Đô thị hoặc Đội cây xanh chuyên nghiệp chăm sóc trong 24 tháng'}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="relative">
                                <div className="absolute -top-3 left-4 bg-pink-500 text-white text-sm font-bold px-3 py-1 rounded-full">
                                    {settings.financeOrgPercent || '10%'}
                                </div>
                                <div className="bg-pink-50 rounded-xl p-6 border-2 border-pink-200 pt-8">
                                    <div className="text-3xl mb-3">🎨</div>
                                    <h4 className="font-bold text-gray-800 mb-2">{settings.financeOrgTitle || 'Tổ Chức & Truyền Thông'}</h4>
                                    <div className="text-2xl font-bold text-pink-600 mb-2">{settings.financeOrgAmount || '50 Triệu'}</div>
                                    <p className="text-sm text-gray-600">
                                        {settings.financeOrgDesc || 'Sản xuất biển tên, in huy hiệu, giấy khen, âm thanh, backdrop Lễ phát động'}
                                    </p>
                                </div>
                            </div>
                        </div>
